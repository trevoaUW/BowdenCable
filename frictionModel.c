...
int main(int argc, char **argv)
{
    ...
    {"P ref: (revs)", 0, 0.0 },
    {"P act: (revs)", 0, 0.0 },
    {"A ref: (revs/s^2)", 0, 0.0},
    {"A act: (revs/s^2)", 0, 0.0},
    {"VDAout: (mV)", 0, 0.0 },
    };
    ...
    // initializing the quadrature encoder counter
    	...
    	while (threadResource->irqThreadRdy == NiFpga_True) {
    		...
    		if(irqAssert & (1 << TIMERIRQNO)) {
    			int ramps = Sramps(mySegs, &iseg, nseg, &itime, T, pref); //rev
    			actualPosition = pos() / 2000; //rev
    			*pact = actualPosition;

    			// check if the user changed the Vref value
    			double error = (*pref - *pact)*2*PI; //rads
            
				//grab aact from physical system, assign to table
				//grab aref from table
    			actualAccel = cascade(*pact, DDERiV, DDERiV_ns,-7.5,7.5);
    			*aact = actualAccel;

				double torqueOut = cascade(error, PIDF, PIDF_ns,-7.5,7.5); //get PID output
				*aref = (1/J)*torqueOut;
				double accelError = (*aref - *aact)*2*PI;//rad/s^2 output of accel. controller (will need motor constants implemented)
				double OutputAccel = cascade(accelError, ACC, ACC_ns,-7.5,7.5);
				double AccelTorqueOutput = OutputAccel * J;

				double vout = AccelTorqueOutput / (Km * Kvi);
				Aio_Write(&CO0,  vout);
    		...
    		MATFILE * mf;
    			...
    			matfile_addstring(mf, "myName", "Tyler Freitas");
            ...
            matfile_addmatrix(mf, "DoubleDervivate", (double *) DERIV, 6, 1, 0);
