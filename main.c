    ...
    // initializing the quadrature encoder counter
    	...
    	while (threadResource->irqThreadRdy == NiFpga_True) {
    		...
    		if(irqAssert & (1 << TIMERIRQNO)) {
    			*pref = *pref*2*PI;
    			int ramps = Sramps(mySegs, nseg, &iseg, &itime, T, pref); //rev
    			actualPosition = pos() / 2000; //rev
    			*pact = actualPosition*2*PI;

    			// check if the user changed the Vref value
    			double error = (*pref - *pact); //rads

				//grab aact from physical system, assign to table
				//grab aref from table
    			actualAccel = cascade(*pact, DDERIV, DDERIV_ns,-1000,1000);
    			*aact = actualAccel;

				double torqueOut = cascade(error, PIDF, PIDF_ns,-7.5,7.5); //get PID output
				*aref = torqueOut / J;
				//double accelError = *aref - *aact;//rad/s^2 output of accel. controller (will need motor constants implemented)
				double accelError = 2;
				double OutputAccel = cascade(accelError, ACC, ACC_ns,-1000,1000);
				double AccelTorqueOutput = OutputAccel * J;

				double vout = AccelTorqueOutput / KT / KVI;
				//double vout = 5;
				Aio_Write(&CO0,  vout);
